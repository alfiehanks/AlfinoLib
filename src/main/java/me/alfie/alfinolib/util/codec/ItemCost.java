...
public record ItemCost(ItemCostIngredient ingredient, int count, Optional<ItemData> data) {
    ...
    public boolean isValid(ItemStack stack) {
        ...
        return data().map(d -> Objects.equals(stack.getTag(), d.tag())).orElse(true);
    }
    ...
    public List<ItemStack> getItemStacks() {
        return getItems().stream()
                .map(item -> {
                    ...
                    data().ifPresent(d -> stack.setTag(d.tag().copy()));
                    return stack;
                })
                .toList();
    }
    ...
    public static final Codec<ItemCost> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
                    ...
                    ItemData.CODEC.optionalFieldOf("nbt").forGetter(ItemCost::data)
            ).apply(instance, ItemCost::new)
    );

    public static final StreamCodec<FriendlyByteBuf, ItemCost> STREAM_CODEC = StreamCodec.of(
            ItemCost::encode,
            ItemCost::decode
    );

    private static void encode(FriendlyByteBuf buf, ItemCost cost) {
    ...
    private static ItemCost decode(FriendlyByteBuf buf) {
